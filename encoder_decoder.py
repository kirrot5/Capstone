...
from .gaze_encoder import PaperStyleGazeEncoder, DualStreamFusion
...
class EncoderDecoder(AttModel):
    def make_model(self, tgt_vocab):
        ...
        rm = RelationalMemory(self.rm_num_slots, self.rm_d_model, self.rm_num_heads)
        model = Transformer(
    ...
    def __init__(self, args, tokenizer):
        ...
        if self.use_eye_gaze:
            ...
            # 融合模块: d_vf是视觉特征维度, d_model是眼动编码后的维度
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks, heatmaps=None, heatmap_masks=None):
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)

        # 如果有眼动数据,进行融合
        if self.use_eye_gaze and heatmaps is not None:
            # 编码眼动热图序列 -> (B, d_model)
            gaze_feats = self.gaze_encoder(heatmaps)
            # 融合到视觉特征 -> (B, R, d_vf)
            att_feats = self.gaze_fusion(att_feats, gaze_feats)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, heatmaps=None, heatmap_masks=None):
