...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        # 修复点：获取模型输出的实际词汇表大小
        output_vocab_size = init_logprobs.shape[-1]

        # INITIALIZATIONS
        beam_seq_table = [torch.LongTensor(batch_size, bdash, 0).to(device) for _ in range(group_size)]
        # 修复点：使用 output_vocab_size 初始化张量
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, output_vocab_size).to(device) for _ in
                                   range(group_size)]
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        # 修复点：使用 output_vocab_size 初始化张量 (self.vocab_size + 1 已经是 output_vocab_size 了)
        beam_seq_logprobs_table = [torch.FloatTensor(self.max_seq_length, bdash, output_vocab_size).zero_() for _ in
                                   range(group_size)]
