...
class R2DataLoader(DataLoader):
    ...
    def collate_fn(self, data):
        if self.use_eye_gaze:
            # 解包7个元素 (包含眼动数据)
            images_id, images, reports_ids, reports_masks, seq_lengths, heatmaps, heatmap_masks = zip(*data)

            # Stack heatmaps
            heatmaps = torch.stack(heatmaps, 0)  # (B, T, H, W)
            heatmap_masks = torch.stack(heatmap_masks, 0)  # (B, T)
        else:
            # 解包5个元素 (不包含眼动数据)
        ...
        if self.use_eye_gaze:
            return images_id, images, torch.LongTensor(targets), torch.FloatTensor(
                targets_masks), heatmaps, heatmap_masks
        else:
